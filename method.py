try:
    import waitGPU
    #waitGPU.wait(utilization=50, memory_ratio=0.5, available_memory=5000, interval=3, nproc=1, ngpu=1)
except ImportError:
    pass
...
def main():
    ...
    parser.add_argument('--probType', type=str, default='im',
        choices=['simple', 'nonconvex', 'acopf57', 'im'], help='problem type')
    ...
    if prob_type == 'im':
        filepath = os.path.join('datasets', 'random_bonelength_im_dataset_fix3_szstart0.75_szend1.25_ex1000')
    else:
...
def train_net(data, args, save_dir):
    ...
    for i in range(nepochs):
        ...
        if valid_loader is not None:
            ...
            # when vaild dataset is None, save placeholders' values
            for k in ['valid_eval','valid_dist','valid_ineq_max','valid_ineq_mean',
                      'valid_ineq_num_viol_0','valid_ineq_num_viol_1','valid_ineq_num_viol_2',
                      'valid_eq_max','valid_eq_mean','valid_eq_num_viol_0',
                      'valid_eq_num_viol_1','valid_eq_num_viol_2','valid_steps','valid_time',
                      'raw_ineq_max','raw_ineq_mean','raw_ineq_num_viol_0','raw_ineq_num_viol_1',
                      'raw_ineq_num_viol_2','raw_eq_max','raw_eq_mean','raw_eq_num_viol_0',
                      'raw_eq_num_viol_1','raw_eq_num_viol_2','raw_time','raw_eval']:
                epoch_stats.setdefault(k, np.array([0.0]))
        ...
        for Xtrain in train_loader:
            ...
            # check if Yhat has penetration
            # if yes, update inequality constraints: G, h
            """
            J_coll, f_coll = optimise.build_penetration_matrix(data.bones_idx, data.radii, Yhat_train, skip_adjacent=True)
            if J_coll is not None and J_coll.shape[0] > 0:
                data.G = torch.tensor(J_coll)
                data.h = torch.tensor(f_coll)
            """
       
            Ynew_train = grad_steps(data, Xtrain, Yhat_train, args)
            # todo: update virtual points
...
def eval_net(data, X, solver_net, args, prefix, stats):
    ...
    # stats for equality constraints, all 0 if no ineq
    if data.neq == 0:
        zeros = np.zeros(B)
        dict_agg(stats, make_prefix('eq_max'), zeros)
        dict_agg(stats, make_prefix('eq_mean'), zeros)
        dict_agg(stats, make_prefix('eq_num_viol_0'), zeros)
        dict_agg(stats, make_prefix('eq_num_viol_1'), zeros)
        dict_agg(stats, make_prefix('eq_num_viol_2'), zeros)

        dict_agg(stats, make_prefix('raw_eq_max'), zeros)
        dict_agg(stats, make_prefix('raw_eq_mean'), zeros)
        dict_agg(stats, make_prefix('raw_eq_num_viol_0'), zeros)
        dict_agg(stats, make_prefix('raw_eq_num_viol_1'), zeros)
        dict_agg(stats, make_prefix('raw_eq_num_viol_2'), zeros)
    else:
    ...
    # stats for inequality constraints, all 0 if no ineq
    if data.nineq == 0:
        zeros = np.zeros(B)
        dict_agg(stats, make_prefix('ineq_max'), zeros)
        dict_agg(stats, make_prefix('ineq_mean'), zeros)
        dict_agg(stats, make_prefix('ineq_num_viol_0'), zeros)
        dict_agg(stats, make_prefix('ineq_num_viol_1'), zeros)
        dict_agg(stats, make_prefix('ineq_num_viol_2'), zeros)

        dict_agg(stats, make_prefix('raw_ineq_max'), zeros)
        dict_agg(stats, make_prefix('raw_ineq_mean'), zeros)
        dict_agg(stats, make_prefix('raw_ineq_num_viol_0'), zeros)
        dict_agg(stats, make_prefix('raw_ineq_num_viol_1'), zeros)
        dict_agg(stats, make_prefix('raw_ineq_num_viol_2'), zeros)
    else:
...
# Used only at train time, do a few steps correction (gradient descent)
def grad_steps(data, X, Y, args):
    if not args['useTrainCorr']:
        return Y
    ...
    # when we want to do correction for ineq, but no completion or no ineq,
    # set partial_corr to False to do the full correction
    if partial_corr and (not partial_var or data.nineq == 0):
        partial_corr = False
    ...
    for _ in range(num_steps):
        if partial_corr:
            ...
            if data.nineq > 0:
                ...
                ineq_step = torch.zeros_like(Y_new)
...
# Used only at test time, do the correction until eq_res is small enough or reach max step num
# so let PyTorch avoid building the computational graph
def grad_steps_all(data, X, Y, args):
    take_grad_steps = args['useTestCorr']
    if not take_grad_steps:
        return Y, 0
    ...
    with torch.no_grad():
        while True:
            # compute eq_max and ineq_max to determine if it has been convergent, only when they exist
            # if data.neq > 0:
            eq_res = torch.abs(data.eq_resid(X, Y_new))            # (B, neq)
            eq_max = torch.max(eq_res) if eq_res.numel() > 0 else torch.tensor(0., device=Y_new.device, dtype=Y_new.dtype)

            if data.nineq > 0:
                ineq_d = data.ineq_dist(X, Y_new)                  # (B, k) or (B, 0)
                if ineq_d.numel() > 0:
                    ineq_max = torch.max(ineq_d)
                else:
                    ineq_max = torch.tensor(0., device=Y_new.device, dtype=Y_new.dtype)
            else:
                ineq_max = torch.tensor(0., device=Y_new.device, dtype=Y_new.dtype)

            if not ( (i == 0) or (eq_max > eps_converge) or (ineq_max > eps_converge) ):
                break
            if i >= max_steps:
                break
